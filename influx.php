...
function influx_shortcodes() {
  add_shortcode('influx_planner', 'influx_shortcode_timetable' );
  add_shortcode('influx_timetable', 'influx_shortcode_timetable' );
}
...
function influx_enqueue_dependencies(){
  ...
  if ( !wp_script_is( 'influx', 'enqueued' ) ) {
    ...
    wp_enqueue_script( 'influx', 'https://cdn.influxhq.com/static/embedinflux.js', array( 'jquery'), $version );
  }
}

function influx_shortcode_timetable() {
  $slug = esc_attr( get_option('influx_slug') );
  $output = "<script type=\"text/javascript\">Influx.timetable({slug:'" . $slug . "'});</script>";
  if(empty($slug)){$output = '<p>Cannot show timetable: Influx slug setting is empty.</p>';}
  return $output;
}
?>